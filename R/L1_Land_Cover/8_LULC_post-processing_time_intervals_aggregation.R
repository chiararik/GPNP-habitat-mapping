...
rfFolder <- file.path(root,"Landsat/RF/ensemble/postprocessing_NEW")

# List all the land cover files in the directory
lc_stack <- rast(list.files(path=rfFolder, pattern=glob2rx("*LC*.tif$"), full.names=TRUE))
...
out_dir <- "D:/ABRESO/PNGP/Landsat/RF/ensemble/postprocessing_NEW"
...
setwd(file.path(root,"Landsat/RF/ensemble/postprocessing_NEW"))
writeRaster(classified_rast, paste0("Pixel_types_NEW.tif"), datatype='INT2S', filetype = "GTiff", overwrite=TRUE)
...
# -------------------------------------------------------------------
# -----------------------------------------------------------
# 1. Define labels and colors for 8 classes
...
c.values <- c("#bdbdbd","#ccece6","#084594",
              "#319812","#024618","#97e800","#74c476",
              "#29c08e")
