...
sel.pred <- c("L4","mask","GS_BSI","GS_CI","GS_GLI","GS_mARI","GS_NBR2","GS_NDBI","GS_NDWI",
...
sel.pred.names <- c("LULC","mask","GS_BSI","GS_CI","GS_GLI","GS_mARI","GS_NBR2","GS_NDBI","GS_NDWI",
                    "GS_SBI","SS_ARVI","SS_BSI","SS_CI","SS_CVI","SS_EVI","SS_GLI",
                    "SS_GVI","SS_GVMI","SS_mARI","SS_MCARI1","SS_NBR2","SS_NDBI",
                    "SS_WET","GS_NIR","GS_SWIR1","SS_blue","SS_NIR","SS_SWIR1",
...
for (y in yy) {
  ...
  all.names <- c("L4",ind.names,bands.names,pred.names,"mask")
  all.pred <- c(lulc,indices,bands,predictors,mask)
  ...
  df$L4 <- as.factor(df$L4)
  gc()
  
  df <- df[,1:32]
  
  # for each class compute statistics 
  for (f in levels(df$L4)[8:30]) {
    
    df.sub <- subset(df, df$mask==substr(f,1,1))
    df.sub <- subset(df, df$L4==f)
    ...
    for (l in 1:(nlyr(all.pred)-1)) {
      ...
      writeRaster(z.stat.part,paste0(root,"/Landsat/z_statistics/habitat_mapping/",y,"_class_",f,"_variable_", names(l),"_zstat_30m_8cl.tif"),
    ...
    setwd(paste0(root,"/Landsat/z_statistics/habitat_mapping"))
    ...
    writeRaster(z.stats, filename=paste0(root,"/Landsat/z_statistics/habitat_mapping/",y,"_class_",f,"_TOT_zstat_30m_8cl.tif"), 
                overwrite=TRUE,datatype="FLT4S",filetype="GTiff")
    ...
    if (sample.size > 50) {
      ...
      writeRaster(training, paste0(root,"/Landsat/z_statistics/habitat_mapping/Training/",y,"_class_",f,"_training_30m_8cl.tif"), datatype='INT2S', filetype = "GTiff", overwrite=TRUE)
