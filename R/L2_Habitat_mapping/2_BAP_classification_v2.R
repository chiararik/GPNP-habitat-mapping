...
sel.pred <- c("L4","GS_BSI","GS_CI","GS_GLI","GS_mARI","GS_NBR2","GS_NDBI","GS_NDWI",
              ...
              "elev","roughness","slope","twi","Geology")
...
train_rf_ensemble <- function(train_data, n_models = 5) {
  ...
  for (i in 1:n_models) {
    ...
    rf_model <- ranger(
      formula = L4 ~ .,  # Adjust formula to match your dataset
      data = boot_data,
      probability = TRUE,
      num.trees = 100,
      mtry = 16,
...
# For each year classify the habitats of the different vegetational macroclasses

for (y in yy[2:length(yy)]) {
  
  for (c in 4:7) {
    
    # Combine all the pixels selected for each habitat class to build the training dataset 
    setwd("D:/ABRESO/PNGP/Landsat/z_statistics/habitat_mapping/Training")
    
    # Function to extract characters at positions 12 and 13
    extract_chars <- function(strings) {
      sapply(strings, function(x) substr(x, 12, 13))
    }
    
    # Apply function and get unique values
    unique_values <- unique(extract_chars(list.files(pattern=glob2rx(paste0(y,"_class_",c,"*training*30m*8cl*")))))
    
    trainings <- rast(list.files(pattern=glob2rx(paste0(y,"_class_",c,"*training*30m*8cl*")))) %>% mask(aoi) 
    ...
    mask <- rast(list.files("D:/ABRESO/PNGP/Landsat/RF/ensemble/postprocessing_NEW", 
                            pattern=glob2rx(paste0(y,'_LC_reclassified.tif$')), all.files=TRUE, full.names=TRUE))
    mask[mask!=c] <- NA
    ...
    df.training <- c(all.trainings,geo,indices,bands,predictors) %>% mask(mask) 
    all.names <- c("L4","Geology",ind.names,bands.names,pred.names)
    ...
    df.train$L4 <- as.factor(df.train$L4)
    df.train$Geology <- as.factor(df.train$Geology)
    
    # mantain only selected predictors and discard the others
    df.train <- select(df.train, all_of(sel.pred))
    dim(df.train)
    t <- df.train %>% group_by(L4) %>% slice_sample(prop = 0.25) %>% tally() %>% 
      as.data.frame()
    print(t)
    ...
    df.train <- df.train %>% group_by(L4) %>% #slice_sample(prop = 0.25) %>% 
      as.data.frame()
    ...
    all.names <- c("Geology",ind.names,bands.names,pred.names)
    names(stack) <- unlist(all.names)
    pngp.stack <- subset(stack, sel.pred[2:length(sel.pred)]) %>% mask(mask)
    pngp.stack$Geology <- as.factor(pngp.stack$Geology)
    ...
    unique_values <- unique(substr(names(all_predictions),2,3))
    # Aggregate the predictions from all models (averaging) ----
    for (d in unique_values) {
      layer_indices <- which(names(all_predictions) == paste0("X",d))
      ...
      setwd("D:/ABRESO/PNGP/Landsat/RF/ensemble/habitat_mapping")
      # Save the averaged probability map
      writeRaster(ensemble_predictions, paste0(y,"_ensemble_class_",d,"_probability.tif"), overwrite = TRUE)
    }
    
    setwd("D:/ABRESO/PNGP/Landsat/RF/ensemble/habitat_mapping")
    ensemble_predictions <- rast(list.files(pattern=glob2rx(paste0(y,"_ensemble_class_",c,"*_probability.tif$"))))
    names(ensemble_predictions) <- unique_values
    
    # Final classification based on the highest probability ----
    final_class <- which.max(ensemble_predictions)
    final_class <- c*10+final_class
    
    # Save the final classification map
    writeRaster(final_class, paste0(y,"_PNGP_class_",c,"habitats_ensemble.tif"), overwrite = TRUE)
...
for (y in yy) {
  ...
  mask <- rast(list.files("D:/ABRESO/PNGP/Landsat/RF/ensemble/postprocessing_NEW", 
                          pattern=glob2rx(paste0(y,'_LC_reclassified.tif$')), all.files=TRUE, full.names=TRUE))
  mask[mask>3&mask<8]<-NA
  
  setwd("D:/ABRESO/PNGP/Landsat/RF/ensemble/habitat_mapping")
  ensemble_habitats <- rast(list.files(pattern=glob2rx(paste0(y,"_PNGP_class_*habitats_ensemble.tif")))) %>% 
    app(fun=sum, na.rm=TRUE)  # Get training variables
  
  ensemble_habitats <- c(ensemble_habitats, mask) %>% 
    app(fun=sum, na.rm=TRUE) # Get training variables
  
  # Save the final classification map
  writeRaster(ensemble_habitats, paste0(y,"_PNGP_habitats.tif"), overwrite = TRUE)
}
